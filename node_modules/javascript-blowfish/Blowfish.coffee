...
# bf = new Blowfish("secret key")
# encrypted = bf.encrypt("конфиденциальное сообщение")
...
# encrypted = bf.encrypt("secret message", "cbcvecto")
# decrypted = bf.decrypt(encrypted, "cbcvecto")
#
...
#
# # Шифрование
# encrypted = bf.base64Encode(bf.encrypt("secret message"))
#
# # Расшифрование
...
class Blowfish
  ...
  constructor: (key, mode) ->
    ...
    @mode = mode if mode in ["ecb", "cbc"]

    # инициализация массивов с кодами символов (правда ведь?)
    @sBox0  = Blowfish.sBox0.slice()
    @sBox1  = Blowfish.sBox1.slice()
    @sBox2  = Blowfish.sBox2.slice()
    @sBox3  = Blowfish.sBox3.slice()
    @pArray = Blowfish.pArray.slice()
  ...
  encrypt: (string, iv) ->
    return @encryptECB(string)      if @mode is "ecb"
    return @encryptCBC(string, iv)  if @mode is "cbc"
  ...
  decrypt: (string, iv) ->
    return @decryptECB(string)      if @mode is "ecb"
    return @decryptCBC(string, iv)  if @mode is "cbc"
  ...
  encryptECB: (string) ->
    ...
    blocks = Math.ceil(string.length / 8)
    ...
    for i in [0 ... blocks]
      ...
      if block.length < 8 and count = 8 - block.length
        block += "\0" while 0 < count--

      [xL, xR] = @encipher(@split64by32(block)...)

      encryptedString += @num2block32(xL) + @num2block32(xR)
  ...
  decryptECB: (string) ->
    ...
    for i in [0 ... blocks]
      ...
      [xL, xR] = @decipher(@split64by32(block)...)

      decryptedString += @num2block32(xL) + @num2block32(xR)

    return @utf8Encode(decryptedString)
  ...
  F: (xL) ->
    ...
    res = @addMod32(@sBox0[a], @sBox1[b])
    res = @xor(res, @sBox2[c])
    res = @addMod32(res, @sBox3[d])
  ...
  generateSubkeys: (key) ->
    ...
    for i in [0 ... 18]
      for j in [4 ... 0]
        data = @fixNegative(data << 8 | key.charCodeAt(k))
        k = (k + 1) % key.length

      @pArray[i] = @xor(@pArray[i], data)
      data = 0

    #block64 = num2block32(0) + num2block32(0);
    block64 = [0, 0];
    for i in [0 ... 18] by 2
      block64 = @encipher(block64...)

      [ @pArray[i], @pArray[i + 1] ] = block64

    for i in [0 ... 256] by 2
      block64 = @encipher(block64...)

      [ @sBox0[i], @sBox0[i + 1] ] = block64

    for i in [0 ... 256] by 2
      block64 = @encipher(block64...)

      [ @sBox1[i], @sBox1[i + 1] ] = block64

    for i in [0 ... 256] by 2
      block64 = @encipher(block64...)

      [ @sBox2[i], @sBox2[i + 1] ] = block64

    for i in [0 ... 256] by 2
      block64 = @encipher(block64...)

      [ @sBox3[i], @sBox3[i + 1] ] = block64

  # Преобразует 4х байтную строку, в 32битное целое число
  # @param {string} block32
  # @return {int}
  block32toNum: (block32) ->
    return @fixNegative(
  ...
  # @return {int}
  addMod32: (a, b) ->
    return @fixNegative((a + b) | 0)  # | 0 приводит к 32битному значению
  ...
  # @return {Array} [xL, xR]
  split64by32: (block64) ->
    xL = block64.substring(0, 4)
    xR = block64.substring(4, 8)

    return [@block32toNum(xL) , @block32toNum(xR)]
